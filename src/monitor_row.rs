...
impl MonitorRow {
    pub fn new(
        ...
        let (contrast_scale, contrast_label, contrast_label_inner, contrast_row) =
            if max_contrast > 0 {
    ...
    {
        if let (Some(ref scale), Some(ref label_inner)) = (
            &self.dynamic_contrast_scale,
            &self.dynamic_contrast_label_inner,
        ) {
            let label_inner = label_inner.clone();
            let callback_clone = callback.clone();
            let adjustment = scale.adjustment();
