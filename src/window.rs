...
impl MainWindow {
    pub fn new(app: &Application) -> Result<Self, DdcError> {
        ...
        let dc_ratio_value_label = Label::new(Some(&format!(
            "{:.1}",
        ...
        for i in 0..monitor_count {
            ...
            let ratio = if settings.borrow().dynamic_contrast_per_monitor_ratio {
                *settings
                    ...
                    .monitor_ratios
                    .get(&name)
                    .unwrap_or(&settings.borrow().dynamic_contrast_ratio)
            } else {
                settings.borrow().dynamic_contrast_ratio
            };
            ...
            if row.has_dynamic_contrast() {
                row.connect_dynamic_contrast_toggle_changed(move |enabled| {
                    settings_clone_toggle
                        .borrow_mut()
                        .monitor_dynamic_contrast
                        .insert(name_clone_toggle.clone(), enabled);
        ...
        dc_enable_switch.connect_state_set(move |_, state| {
            ...
            for row in monitor_rows_enable.borrow().iter() {
                ...
                row.set_dynamic_contrast_toggle_visible(
                    state && !settings_enable.borrow().dynamic_contrast_global,
                );
            }
            Propagation::Proceed
        });
        ...
        dc_per_monitor_switch.connect_state_set(move |_, state| {
            settings_pm_switch
                .borrow_mut()
                .dynamic_contrast_per_monitor_ratio = state;
